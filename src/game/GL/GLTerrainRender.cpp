...
const char *GLRenderEngine::GLTerrainDriverName(){
	static const char DriverName[] = "Standard Terrain Driver";
	return DriverName;
}
...
struct BinaryTriangle{
	BinaryTriangle *LeftNeighbor, *RightNeighbor, *BottomNeighbor, *LeftChild, *RightChild;
...
inline void FreeBinTri(BinaryTriangle *tri){
	return;
}
...
inline void BinaryTriangle::Split(){
	if(BottomNeighbor == NULL){	//No bottom, so split half-diamond.
		...
		if(BottomNeighbor->BottomNeighbor != this){	//Not diamond, so split bottom neighbor.
			...
			BottomNeighbor->Split();
		}
		//Now BottomNeighbor->BottomNeighbor _should_ == this...
		if(BottomNeighbor->BottomNeighbor == this && AvailBinTris() >= 4){
			Split2();
...
void BinaryTriangle::TestSplit(LodTree *lod, int variance, int level, int index){
	if(lod->lodtree[LODTREEOFF(std::min(level, LodTreeDepth - 1)) + index] > variance){
		...
		if(level < (MaxLodDepth - 1) - LimitLod){
			...
			level++;
			if(LeftChild) LeftChild->TestSplit(lod, variance, level, index);
			if(RightChild) RightChild->TestSplit(lod, variance, level, index2);
		}
	}
};
...
void BinaryTriangle::TestSplitZ(int level, int index, float cz1, float cz2, float cz3){
	...
	if(vd > iv){
		...
		if(level < (MaxLodDepth - 1) - LimitLod){
			...
			level++;
			if(LeftChild) LeftChild->TestSplitZ(level, index, cz3, cz1, avgcz);
			if(RightChild) RightChild->TestSplitZ(level, index2, cz2, cz3, avgcz);
			return;
		}
	}else{
	...
	if(t > radius && t2 > radius){	//Unclipped totally.
		...
		if(vd > iv){
			...
			if(level < (MaxLodDepth - 1) - LimitLod){
				...
				if(LeftChild) LeftChild->TestSplitZ(level, index, t3, t1, avgcz);
				if(RightChild) RightChild->TestSplitZ(level, index2, t2, t3, avgcz);
				return;
			}
		}else{
			SplitVertHeight = 0.0f;
		...
		if(vd > iv){
			...
			if(level < (MaxLodDepth - 1) - LimitLod){
				...
				level++;
				radius *= 0.707107f;
				if(LeftChild) LeftChild->TestSplitClip(level, index, radius, x3, y3, x1, y1, avgx, avgy);
				if(RightChild) RightChild->TestSplitClip(level, index2, radius, x2, y2, x3, y3, avgx, avgy);
				return;
			}
		}else{
			SplitVertHeight = 0.0f;
...
struct MapPatch{
	...
	void Split(float variance){//, int scale
		...
		//
		lod = lodul;
		ul.TestSplitClip(0, 0, rad, x1, y2, x2, y1, x1, y1);
		lod = loddr;
		dr.TestSplitClip(0, 0, rad, x2, y1, x1, y2, x2, y2);
	};
};
